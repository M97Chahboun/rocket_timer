  ...
  group('RocketTimer tests', () {
    test('Test initial values', () {
      RocketTimer timer = RocketTimer(
          duration: 0, status: TimerStatus.initial, type: TimerType.normal);
    ...
    test('Test incrementing timer in normal mode', () {
      RocketTimer timer = RocketTimer(duration: 0, type: TimerType.normal);
      timer.start();
      Future.delayed(const Duration(seconds: 10), () {
        timer.pause();
        expect(timer.duration, 10);
        expect(timer.hours, 0);
        expect(timer.minutes, 0);
    ...
    test('Test decrementing timer in countdown mode', () {
      RocketTimer timer = RocketTimer(duration: 60, type: TimerType.countdown);
      timer.start();
      Future.delayed(const Duration(seconds: 10), () {
        expect(timer.duration, 50);
        expect(timer.hours, 0);
        expect(timer.minutes, 0);
    ...
    test('Test switching between countdown and normal modes', () {
      RocketTimer timer = RocketTimer(duration: 60, type: TimerType.normal);
      expect(timer.type, TimerType.normal);
